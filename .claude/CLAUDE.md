...
`scripts/tests/` holds 8 `node:test` files (49 cases). No `test` npm script — run them with `node --test "scripts/tests/*.test.cjs"` (quote the glob; a directory argument fails on Node 25 / Windows). Six compile the target `.ts` with `tsc` and assert on real behavior; two (`homeSwipeDelete`, `videoDetailKeyboardAccessory`) match **source text via regex**, so a refactor can break them while the app still works. Whether the plain compile works depends on the target's imports — an aliased (`@/`) import needs a self-contained temp tsconfig, and relative imports nest the emit under `out/utils/`. Date tests pin `process.env.TZ`; verify new ones with `TZ=UTC`. See `.memory/testing.md`. The `expo-symbols` / `expo-haptics` choke points are enforced by `no-restricted-imports` in `eslint.config.js`, not by these tests.
