...
src/constants/   colors, icons, techniques, diaryOptions, skiResorts.json (378 resorts). `Colors` has exactly one home: `colors.ts`.
...
- **i18n is device-locale-only (no runtime switching)** — `src/i18n/index.ts` reads `expo-localization.getLocales()[0]?.languageCode` once at module init, normalises non-`ja` to `en`, and pins `i18n-js`'s locale to that value. `useTranslation()` returns `{ t, locale }` only — there is no preference / setter API, no persisted `locale` key, and the in-app language picker was removed in `bab0b45`. Users change language by changing iOS system language, which restarts the app — so `useTranslation` has no subscription machinery at all. Type parity between `locales/ja.ts` and `locales/en.ts` is still enforced by `Translations = typeof ja`.
...
- `app_preferences` is a key-value store. Keys actually in use today: `capturedAt_repair_version`, `thumbnail_migration_version`, `home_sort_order`, `weekStartDay` (camelCase by historical accident — do not migrate to snake_case without a data migration), `dismissed_update_prompt_version`. The stale `app_locale` row left by the removed language picker (`bab0b45`) is deleted at startup next to `seedTechniqueOptions`; the key was `app_locale`, never `locale`. Note `exportService` dumps every preference verbatim, so any key added here lands in the backup JSON too.
