...
## Testing Guidelines
`scripts/tests/` holds 5 `node:test` files (18 cases). There is no `test` npm script — run them with `node --test "scripts/tests/*.test.cjs"` (quote the glob; a directory argument fails on Node 25 / Windows). They assert on source text via regex rather than behavior, so refactors can break them while the app still works, and one case in `homeSwipeDelete.test.cjs` fails on `main` as of 2026-07-15. Take a baseline before assuming your change broke it. See `.memory/testing.md`.
