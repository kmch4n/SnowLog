...
![App Store](https://img.shields.io/badge/App%20Store-2026%2F01%20release-0A84FF)
![Version](https://img.shields.io/badge/version-1.2.0-brightgreen)
![Expo SDK](https://img.shields.io/badge/Expo%20SDK-55-000020)
![React Native](https://img.shields.io/badge/React%20Native-0.83.6-61DAFB)
...
1 分 42 秒の紹介映像である。主要な画面と、参照型ストレージ・オフラインファーストという設計上の判断を通して見せている。
...
**開発期間**: 2025 年 12 月開始、継続改善中。**App Store 初公開日**: 2026 年 1 月。
...
- **一括操作 / データエクスポート** — 長押し選択モードによる一括削除、および全データの JSON エクスポート（`expo-sharing` 経由）を備える。ホーム画面では並び順切替（最新 / 最古 / ゲレンデ順）とスワイプ削除に対応する。
- **日英 2 言語対応** — `i18n-js` + `expo-localization` でデバイスロケールを起動時に一度だけ判定する（`ja` 以外は `en`）。アプリ内に言語切替 UI は無く、iOS の設定で端末の言語を変更する。iOS InfoPlist のパーミッション文言も `expo.locales` と `CFBundleLocalizations` 経由でローカライズしている。
- **ハプティクスフィードバック** — お気に入りトグル・一括選択開始・削除確定・インポート完了等の主要操作に `expo-haptics` を配線し、ネイティブ流儀の触覚フィードバックを提供する。
...
| [i18n-js](https://github.com/fnando/i18n) v4 + [expo-localization](https://docs.expo.dev/versions/latest/sdk/localization/) | 国際化 | App Store の非日本語ストア向けフォールバック表示と、アプリ内 UI の日英切替を統一エンジンで提供するため |
...
- **ナビゲーション構成**: 「ホーム / ダッシュボード / カレンダー / 検索 / 設定」の 5 タブに定着するまでに、ホーム単一画面から機能を分離する過程で数度組み換えた。最終的には Expo Router の `NativeTabs` + Liquid Glass タブバーに移行した。
...
- **Android 対応（将来検討）** — 現在は iOS 向けに App Store で配布している。iOS ユーザーが増え、プロダクトとして十分に成長した段階で Android 版を検討する。
