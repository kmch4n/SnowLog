...
- **日英 2 言語対応** — `i18n-js` + `expo-localization` でデバイスロケールを自動判定する。設定画面からの手動切替（日本語 / English / デバイスに従う）に対応し、選択は端末ローカルに永続化する。iOS InfoPlist のパーミッション文言も `expo.locales` 経由でローカライズしている。
